...
/*     create table &RDP._missed_index_&NUM. as select a.* from indexprenataloutcomes_&NUM. a left join &RDP._index_outcome_fin_&NUM. b*/
		...
		left join &RDP._index_outcome_fin_&NUM. b
       	on a.patient_deid=b.patient_deid and a.idxpren=b.idxpren /*CDL: 1.19.2025 -- Added the requirement for the patient_deid*/
	        	...
	        	%end;      
	         	...
	         	if PREG_Outcome in ('LBS' 'LBM' 'MLS') then do;
	              	if outnext_assigned in ('LBS' 'LBM' 'MLS') and days_outEndnextStart < 168 then SamePregnancy=1;
	              		...
	              		else if outnext_assigned in ('EM' 'AEM') and days_outEndnextStart < 70 then SamePregnancy=1; /*CDL: ADDED AEM 11.25.2024*/
	         	...
	         	else if PREG_Outcome in ('EM' 'AEM') then do; /*CDL: ADDED AEM 11.25.2024*/
	              	...
	              	else if outnext_assigned in ('EM' 'AEM') and days_outEndnextStart < 56 then SamePregnancy=1; /*CDL: ADDED AEM 11.25.2024*/
	          		...
	          		if outnext_assigned in ('LBS' 'LBM' 'MLS') and days_outEndnextStart < 140 then SamePregnancy=1;
	              		...
	              		else if outnext_assigned in ('EM' 'AEM') and days_outEndnextStart < 42 then SamePregnancy=1; /*CDL: ADDED AEM 11.25.2024*/
	              		else if outnext_assigned in ('SAB' 'IAB' 'UAB') and days_outEndnextStart < 42 then SamePregnancy=1;
	              		else if outnext_assigned in ('UDL') and days_outEndnextStart < 140 then SamePregnancy=1; /*CDL: CHANGED TO 140 from 154. Was a typo in the protocol. 11.25.2024. Fixed protocol.*/
	         	end;
