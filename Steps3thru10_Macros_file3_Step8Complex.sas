...
/*	  proc contents data= rpgroups_enc_&p.; run;*/
...
/*		data test;*/
/*		set preg_2cb_&p.;*/
/*			if Dt_PrenEnc1st ne Dt_PG_Index then pnc_wrong = 1;*/
/*				else pnc_wrong = 0;*/
/*			if Dt_PrenEncLast ne DT_LTFU then outc_wrong = 1;*/
/*				else outc_wrong = 0;*/
/*		run;*/
/*		proc freq data=test; table pnc_wrong*outc_wrong; run;*/
...
/*		proc freq data=test; table pnc_wrong*outc_wrong; run;*/
	...
	/*%let dd = 1;*/
		...
		***-------------------------------------------------------------------------;
		...
		proc sql;
			select count(*) as n_row, count(distinct patient_deid) as n_id
    ...
    *08.05 - correct dsn name - should be _COMP not _Complex;
  	data pregnancy_prenatalonly_COMP; *need to modify Step10 if use dsn other than Pregnancy_PrenatalOnly (done 07.01);
   	set %do i = 1 %to &p.-1 ; Prenpregnancies_&i. %end; ;
		...
		/*CDL: ADDED lines to add back pregnancy outcome, dropped earlier -- all are UNK*/
		PREG_Outcome = 'UNK';
		PREG_OUTCOME_Clean = 'UNK';

	    keep patient_deid dt_indexprenatal idxpren status dt_ltfu ltfu_assignment ltfu_step
	          dt_prenenc1st dt_prenenclast PREG_Outcome PREG_OUTCOME_Clean;
  	run;
