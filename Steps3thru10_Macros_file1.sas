...
%macro PrepOCG;
	...
	/*Double check count*/
/*	proc sql;*/
/*		select count(distinct patient_deid) as n_ids from out.outcomeassigned_&OCW.;*/
/*		quit;*/

	/*Derive some necessary datasets with flags for prenatal care and pregnancy outcomes*/
    proc sql;

		/*Merge the prenatal care dates with a flag to indicate that the person has at least one pregnancy outcome group*/
    ...
    set _prenataldates_all;
	  	...
	  	/*The row number of prenatal encounter dates among everyone in the dataset*/
      	PRENNUM = _n_;
    ...
    proc sql noprint;
       	...
       	/*get maximum num of prenatal index dates avail per person*/
...
%macro Step34_Link_IdxOCG;
    ...
    set prenataldates_OUTC_%eval(&NUM. - 1) ;  %*prenataldates - prenataldates_updt_#;
    	by patient_deid;
      	...
      	if first.patient_deid then do;
       		IDXPREN= _n_ + (&num. * 1000000); /*Create a ID for the indexing prenatal encounter thats dependent on iteration*/
	...
	/*Check: 
	proc sql;
		select count(distinct patient_deid) as id_count from prenataldates_outc_%eval(&num. -1);
		quit;
	Find: id_count is equal to the number of rows in IndexPre_&num. for testing parameters
	*/
    ...
    **Identify all outcome groups that overlap index prenatal bounds (indexdt + POW);
	...
	/*Check: Code this myself, ignoring variables
	proc sql;
		create table test as
		select distinct a.*, b.*
		from IndexPre_&num. as a
		left join OUTCOMEASSIGNED_PREN_%eval(&NUM. - 1) as b
		on a.patient_deid=b.patient_deid and (a.Dt_IndexPrenatal <= b.dt_outcomegroup_start <= a.Dt_IndexPrenatal+&pow. OR
							a.Dt_IndexPrenatal <= b.dt_outcomegroup_end <= a.Dt_IndexPrenatal+&pow.)
		;
		quit;
	Finding: Same number of rows for n=1! 
	proc compare base=IndexPrenatalOutcomes_&Num. compare=test;
	run;
	Finding: Shared variables are the same in all rows for n=1.
	*/
...
%macro Step5cd_PregTimeline;
    ...
    *now use dsn index prenatal and pregnancy outcome based on the outcomegrps w/in 286d of index w/in table3 gaps;
    ...
    set s5_index_outcome_fin_&num. ; 
			/*s5_index_outcome_fin_&num. is the final dataset of index prenatal encounters with pregnancy outcome groups for the
					CURRENT indexing prenatal encounter (i.e., not ALL indexing prenatal encounters.*/
...
/*	proc sql;*/
/*		create table test as*/
/*		select a.*, b.resultant_0*/
/*		from Index_Outcome_Fin_Result_&NUM. as a*/
/*		left join &RDP._outidsfin_&NUM. as b*/
/*		on a.patient_deid=b.patient_deid and a.prennum=b.prennum*/
/*		;*/
/*		quit;*/

	/*Same answer both ways*/
/*	proc sql;*/
/*		select sum((dt_pregnancylookback le dt_indexprenatal le dt_pregnancyoutcome_start) or */
/*				(dt_pregnancyoutcome_start le dt_indexprenatal le dt_pregnancyoutcome_end)) as n_outc,*/
/*				sum(dt_indexprenatal lt Dt_PregnancyLookback) as n_ltfu*/
/*		from Index_Outcome_Fin_Result_&NUM.*/
/*		;*/
/*		quit;*/
/*	proc freq data=Index_Outcome_Fin_Result_&NUM.;*/
/*		table index_finding_&num. / missing;*/
/*	run;*/


    *** 5D - those indexing prenatal encounters without any pregnancy outcome groups in the 286 days ;
    **     - rephrase - pull out index prenatals with no outcome and label those as missing/unknown;
    **       first by pooling all Dt_IndexPrenatal and then assigning label;
    proc sql;
     	create table Final_Indexpre_&NUM.  as
      	select a.*, PREG_OutcomeCount, PREG_OutcomesCompared, Preg_outcomeCodetypesCOmpared, PREG_Outcome,PREG_CodeType, 
					PREG_MIFE, PREG_MISO, dt_pregnancyoutcome_start, dt_pregnancyoutcome_end, DT_PREG_Outcome,
					...
					Any rows not in the final dataset means that a pregnancy outcome group was NOT linked to the indexing
					prenatal encounter in the prior evaluation steps*/
					case when missing(b.idxpren) then "tbd" else Index_Finding_&NUM. 
					end as Index_Finding_&NUM., 
      	...
      	from (select distinct patient_deid, idxpren, PRENNUM, dt_indexprenatal, outcomegroupsfound 
				from indexprenataloutcomes_&NUM.) a /*This is the dataset of ALL pregnancy outcome groups within POW of the indexing PNC encounter*/
        left join index_outcome_fin_result_&NUM. b /*Final results for the CURRENT indexing prenatal encounter*/
...
/*		*Calculate myself:;*/
/*		proc sql;*/
/*			select count(*) as n_tbd from indexprenataloutcomes_&NUM. where patient_deid not in (select patient_deid from index_outcome_fin_result_&NUM.);*/
/*			select sum(DT_INDEXPRENATAL < Dt_PregnancyLookback) as n_ltfu, sum(not(DT_INDEXPRENATAL < Dt_PregnancyLookback)) as n_outc from index_outcome_fin_result_&NUM.;*/
/*			quit;*/
/*		*Both methods return the same number of rows.;*/
...
/*	proc sql;*/
/*		select sum(PREG_OUTCOME="") from final_indexpre_&num; */
/*		quit; */
/*	*This is the sum of those TBD and LTFU, which is correct.;*/
/*	proc freq data=final_indexpre_&num;*/
/*		table PREG_OUTCOME / missing;*/
/*	run;*/
...
/*	proc sql;*/
/*		create table tbd as*/
/*		select a.*, case when missing(b.prennum) then "AVAIL" else "NO_OUTC" */
/*					end as status_1 format $20.*/
/*		from (select * */
/*				from PRENATALDATES_OUTC_%eval(&NUM. - 1) */
/*				where patient_deid in (select distinct patient_deid from FINAL_INDEXPRE_&NUM. where Index_Finding_1='tbd')) as a /*all prenatal encounters to that person*/*/
/*		left join (select * from FINAL_INDEXPRE_&NUM. where Index_Finding_1='tbd') as b*/
/*		on a.patient_deid=b.patient_deid and a.prennum=b.prennum*/
/*		;*/
/*		quit;*/
/**/
/*	*Second, identify those prenatal encounters that were linked to an outcome and then grab all PNC in that range;*/
/*	proc sql;*/
/*		create table outc as*/
/*		select a.*, b.DT_INDEXPRENATAL, b.index_finding_1, b.Dt_PregnancyOutcome_End, b.Dt_PregnancyLookFwd*/
/*		from (select **/
/*				from PRENATALDATES_OUTC_%eval(&NUM. - 1) */
/*				where patient_deid in (select distinct patient_deid from FINAL_INDEXPRE_&NUM. where Index_Finding_1 = 'OUTC')) as a	*/
/*		left join (select * from FINAL_INDEXPRE_&NUM. where index_finding_1='OUTC') as b*/
/*		on a.patient_deid=b.patient_deid*/
/*		;*/
/*		quit;*/
/*	data outc2;*/
/*	set outc;*/
/*		format status_1 $20.;*/
/*		if dt_indexprenatal <= enc_date <= dt_pregnancyoutcome_end then STATUS_1 = 'PREN_OC_&NUM';*/
/*			else if dt_pregnancyoutcome_end < enc_date <= Dt_PregnancyLookFwd then STATUS_1 = 'PREN_OC_&NUM POST';*/
/*			else STATUS_1 = 'AVAIL';*/
/*	run;*/
/**/
/*	*Third, deal with those LTFU;*/
/*	*First, identify those prenatal encounters that have no outcome and all subsequent need to be dealt with - TBD;*/
/*	proc sql;*/
/*		create table ltfu as*/
/*		select a.*, case when missing(b.prennum) then "AVAIL" else "LTFU" */
/*						end as STATUS_1 format $20.*/
/*		from (select * */
/*				from PRENATALDATES_OUTC_%eval(&NUM. - 1) */
/*				where patient_deid in (select distinct patient_deid from FINAL_INDEXPRE_&NUM. where Index_Finding_1='LTFU')) as a /*all prenatal encounters to that person*/*/
/*		left join (select * from FINAL_INDEXPRE_&NUM. where Index_Finding_1='LTFU') as b*/
/*		on a.patient_deid=b.patient_deid and a.prennum=b.prennum*/
/*		;*/
/*		quit;*/
/**/
...
%MAcro OGP_Updt;
            ...
            **need this to find outcomegrp evaluated for index prenatals later marked LTFU;
            proc sql;
             	create table S5_outidsfin_&num._L as /*These are all the pregnancy outcome groups that were evaluated*/
						...
						/*Create RESULT_NUM for all of the pregnancy outcome groups*/
                     	CASE
                         	/*if the outcomegrp has already been linked to prenatal index keep previous finding/result (only iteration 2+)*/
                          	WHEN A.RESULT_%eval(&NUM. - 1) Not in ("find index") THEN a.RESULT_%eval(&NUM. - 1)
							...
							/*if outcome group occurred before PNC encounter, then before - CDL: MOVED UP 11.25.2024*/
							WHEN .Z < A.DT_OUTCOMEGROUP_END < C.DT_INDEXPRENATAL THEN "BEFORE PREN_OC_&NUM."

							/*CDL: MODIFIED to track more closely with protocol 11.25.2024*/
...
/*                          	when a.outcomegrp=d.outcomegrp and d.index_finding_&num. in ("LTFU") then "find index"*/
							when c.index_finding_&num in ("tbd","LTFU") then "find index"
							...
							when a.Dt_OutcomeGroup_Start <= c.dt_indexprenatal or a.Dt_OutcomeGroup_End <= c.Dt_PregnancyOutcome_End then "PREN_OC_&NUM."
...
/*                          	WHEN .Z< A.DT_OUTCOMEGROUP_END < C.DT_INDEXPRENATAL THEN "BEFORE PREN_OC_&NUM." */
                         	...
                         	/*or is sufficiently far from pregnancy end date to be another/new pregnancy (after alloted time) (step 6 + table 3)*/
                          	WHEN C.PREG_Outcome in ('LBS' 'LBM' 'MLS' 'SB' 'UDL') And /*CDL: ADDED UDL 11.25.2024*/
                               	A.Outcome_assigned&ALG. in ('LBS' 'LBM' 'MLS' 'SB' 'UDL') and /*.Z*/ 0 < CALCULATED Days_FromPregEnd < 168 then "PREN_OC_&NUM. EXT"
                          	WHEN C.PREG_Outcome in ('LBS' 'LBM' 'MLS' 'SB' 'UDL') And /*CDL: ADDED UDL 11.25.2024*/
                               	A.Outcome_assigned&ALG. in ('AEM' 'EM' 'SAB' 'IAB' 'UAB') and /*.Z*/ 0 < CALCULATED Days_FromPregEnd < 70 then "PREN_OC_&NUM. EXT"
								/*CDL: ADDED AEM 11.25.2024*/

                          	WHEN C.PREG_Outcome in ('EM' 'AEM') And /*CDL: ADDED AEM 11.25.2024*/
                               	A.Outcome_assigned&ALG. in ('LBS' 'LBM' 'MLS' 'SB' 'UDL') and /*.Z*/ 0 < CALCULATED Days_FromPregEnd < 154 then "PREN_OC_&NUM. EXT"
                          	WHEN C.PREG_Outcome in ('EM' 'AEM') And /*CDL: ADDED AEM 11.25.2024*/
                               	A.Outcome_assigned&ALG. in ('AEM' 'EM' 'SAB' 'IAB' 'UAB') and /*.Z*/ 0 < CALCULATED Days_FromPregEnd < 56 then "PREN_OC_&NUM. EXT"

                          	WHEN C.PREG_Outcome in ('SAB' 'IAB' 'UAB' ) And
                               	A.Outcome_assigned&ALG. in ('LBS' 'LBM' 'MLS' 'SB' 'UDL') and /*.Z*/ 0 < CALCULATED Days_FromPregEnd < 140 then "PREN_OC_&NUM. EXT"
                          	WHEN C.PREG_Outcome in ('SAB' 'IAB' 'UAB' ) And
                               	A.Outcome_assigned&ALG. in ('AEM' 'EM' 'SAB' 'IAB' 'UAB') and /*.Z*/ 0 < CALCULATED Days_FromPregEnd < 42 then "PREN_OC_&NUM. EXT" /*CDL: ADDED AEM 11.25.2024*/
                          	WHEN C.PREG_Outcome in ('SAB' 'IAB' 'UAB' ) And
                               	A.Outcome_assigned&ALG. in ('UDL') and /*.Z*/ 0 < CALCULATED Days_FromPregEnd < 140 then "PREN_OC_&NUM. EXT" /*CDL: CHANGED 154 to 140. Was a typo in protocol. Fixed here and in protocol. 11.25.2024*/

							/*CDL: These lines are now redundant - 11.25.2024*/
                          	WHEN C.PREG_Outcome in ('UDL') And
                               	A.Outcome_assigned&ALG. in ('LBS' 'LBM' 'MLS' 'SB' 'UDL') and /*.Z*/ 0 < CALCULATED Days_FromPregEnd < 168 then "PREN_OC_&NUM. EXT"
                          	WHEN C.PREG_Outcome in ('UDL') And
                               	A.Outcome_assigned&ALG. in ('AEM' 'EM' 'SAB' 'IAB' 'UAB') and /*.Z*/ 0 < CALCULATED Days_FromPregEnd < 70 then "PREN_OC_&NUM. EXT" /*CDL: ADDED AEM 11.25.2024*/
                     	...
                     	CASE when Calculated Result_&NUM. in ("PREN_OC_&NUM EXT","BEFORE PREN_OC_&NUM.","MID PREN_OC_&NUM.") then c.idxpren /*capture from indexprenatal preg info*/
                          	when Calculated Result_&NUM.= "PREN_OC_&NUM." then c.idxpren /*b.idxpren*/ /*CDL: MODIFIED 11.26.2024 - Was pulling in the wrong pregnancy info*/
									/*from pair evaluation not ltfu*/
                     	...
                     	CASE when Calculated Result_&NUM. in ("PREN_OC_&NUM EXT","BEFORE PREN_OC_&NUM.","MID PREN_OC_&NUM.") then c.dt_indexprenatal /*capture from indexprenatal preg info*/
                          	when Calculated Result_&NUM.= "PREN_OC_&NUM." then c.dt_indexprenatal /*b.dt_indexprenatal*/ /*CDL: MODIFIED 11.26.2024 - Was pulling in the wrong pregnancy info*/
									/*from pair evaluation not ltfu*/
                          	when Calculated Result_&NUM. ="find index" then  . /*nada, no prenatal preg info yet*/
                          	%if &num.>1 %then %do;
                     	...
                     	CASE when Calculated Result_&NUM. in ("PREN_OC_&NUM EXT","BEFORE PREN_OC_&NUM.","MID PREN_OC_&NUM.") then c.dt_pregnancyoutcome_start /*capture from indexprenatal preg info*/
                          	when Calculated Result_&NUM.= "PREN_OC_&NUM." then c.dt_pregnancyoutcome_start /*b.dt_pregnancyoutcome_start*/   /*CDL: MODIFIED 11.26.2024 - Was pulling in the wrong pregnancy info*/
									/*from pair evaluation not ltfu*/
                          	when Calculated Result_&NUM.= "find index" then .   /*nada, no prenatal preg info yet*/
                     	...
                     	CASE when Calculated Result_&NUM. in ("PREN_OC_&NUM EXT","BEFORE PREN_OC_&NUM.","MID PREN_OC_&NUM.") then c.dt_pregnancyoutcome_end /*capture from indexprenatal preg info*/
                          	when Calculated Result_&NUM. ="PREN_OC_&NUM." then c.dt_pregnancyoutcome_end /*b.dt_pregnancyoutcome_end*/ /*CDL: MODIFIED 11.26.2024 - Was pulling in the wrong pregnancy info*/
									/*from pair evaluation not ltfu*/
                          	when Calculated Result_&NUM. ="find index" then .   /*nada, no prenatal preg info yet*/
                     	...
                     	CASE when Calculated Result_&NUM. in ("PREN_OC_&NUM EXT","BEFORE PREN_OC_&NUM.","MID PREN_OC_&NUM.") then c.dt_PregnancyLookback /*capture from indexprenatal preg info*/
                          	when Calculated Result_&NUM.= "PREN_OC_&NUM." then c.dt_PregnancyLookback /*b.dt_PregnancyLookback*/ /*CDL: MODIFIED 11.26.2024 - Was pulling in the wrong pregnancy info*/
									/*from pair evaluation not ltfu*/
                          	when Calculated Result_&NUM.= "find index" then .   /*nada, no prenatal preg info yet*/
                          	when calculated Result_&NUM. contains "PREN_OC" then . /*previously found indexpren*/
                     	...
                     	CASE when Calculated Result_&NUM. in ("PREN_OC_&NUM EXT","BEFORE PREN_OC_&NUM.","MID PREN_OC_&NUM.") then c.dt_PregnancyLookFwd /*capture from indexprenatal preg info*/
                          	when Calculated Result_&NUM.= "PREN_OC_&NUM." then c.dt_PregnancyLookFwd /*b.dt_PregnancyLookFwd*/ /*CDL: MODIFIED 11.26.2024 - Was pulling in the wrong pregnancy info*/
									/*from pair evaluation not ltfu*/
                          	when Calculated Result_&NUM.= "find index" then .   /*nada, no prenatal preg info yet*/
                          	when calculated Result_&NUM. contains "PREN_OC" then . /*previously found indexpren*/
                     	...
                     	CASE when  Calculated Result_&NUM. in ("PREN_OC_&NUM EXT" "BEFORE PREN_OC_&NUM." "MID PREN_OC_&NUM.") then c.PREG_Outcome /*capture from indexprenatal preg info*/
                          	when  Calculated Result_&NUM. ="PREN_OC_&NUM." then c.PREG_Outcome /*b.PREG_Outcome*/ /*CDL: MODIFIED 11.26.2024 - Was pulling in the wrong pregnancy info*/
									/*from pair evaluation not ltfu*/ 
                          	when  Calculated Result_&NUM. ="find index" then ''   /*nada, no prenatal preg info yet*/
                          	when calculated Result_&NUM. contains "PREN_OC" then '' /*previously found indexpren (info in prev file)*/
                     	...
                     	CASE when  Calculated Result_&NUM. in ("PREN_OC_&NUM EXT" "BEFORE PREN_OC_&NUM." "MID PREN_OC_&NUM.") then c.dt_PREG_Outcome /*capture from indexprenatal preg info*/
                          	when  Calculated Result_&NUM. = "PREN_OC_&NUM." then c.dt_PREG_Outcome /*b.dt_PREG_Outcome*/ /*CDL: MODIFIED 11.26.2024 - Was pulling in the wrong pregnancy info*/
								/*from pair evaluation not ltfu*/
                          	when  Calculated Result_&NUM. = "find index" then .   /*nada, no prenatal preg info yet*/
                          	when calculated Result_&NUM. contains "PREN_OC" then . /*previously found indexpren*/
              	...
              	FROM OUTCOMEASSIGNED_PREN_%eval(&NUM. - 1) A  /*data for all outcome groups from the last evaluation*/
                LEFT JOIN FINAL_INDEXPRE_&NUM. C /*Join the information for the final pregnancies identified*/
				ON A.PATIENT_DEID = C.PATIENT_DEID                              /*result for all index prenatal eval of outc grps (incl 0 outc)*/
                left join (select * 
							from s5_outidsfin_&num._l
                ...
                left join (select patient_deid, outcomegrp, index_finding_&num. 
							from s5_outidsfin_&num._l
...
/**/
/*	*CDL coded separately to check SPH code - Modified based upon discrepancies.;*/
/**/
/*	proc sql;*/
/*		create table outc_grp as*/
/*		select a.*, b.index_finding_1, b.dt_PregnancyOutcome_Start, b.dt_PregnancyOutcome_End, b.PREG_Outcome, b.OutcomeGroupsFound, b.dt_indexprenatal*/
/*		from outcomeassigned_pren_%eval(&NUM. - 1) as a*/
/*		left join final_indexpre_&num as b*/
/*		on a.patient_deid = b.patient_deid*/
/*		;*/
/*		quit;*/
/**/
...
/*	set outc_grp;*/
/**/
/*		days_from_preg_end =  Dt_OutcomeGroup_Start - Dt_PregnancyOutcome_End ;*/
/**/
/*		if Dt_PregnancyOutcome_End < Dt_OutcomeGroup_Start then do;*/
/**/
/*			if PREG_Outcome in ('LBM','LBS','MLS','SB','UDL') and Outcome_Assigned1 in ('LBM','LBS','MLS','SB','UDL') and 0 < days_from_preg_end < 168 then tooclose=1;*/
/*				else if PREG_Outcome in ('LBM','LBS','MLS','SB','UDL') and Outcome_Assigned1 in ('SAB','IAB','UAB','EM','AEM') and 0 < days_from_preg_end < 70 then tooclose=1;*/
/*				else if PREG_Outcome in ('SAB','IAB','UAB') and Outcome_Assigned1 in ('LBM','LBS','MLS','SB','UDL') and 0 < days_from_preg_end < 140 then tooclose = 1;*/
/*				else if PREG_Outcome in ('SAB','IAB','UAB') and Outcome_Assigned1 in ('SAB','IAB','UAB','EM','AEM') and 0 < days_from_preg_end < 42 then tooclose = 1;*/
/*				else if PREG_Outcome in ('AEM','EM') and Outcome_Assigned1 in ('LBM','LBS','MLS','SB','UDL') and 0 < days_from_preg_end < 154 then tooclose=1;*/
/*				else if PREG_Outcome in ('AEM','EM') and Outcome_Assigned1 in ('SAB','IAB','UAB','EM','AEM') and 0 < days_from_preg_end < 56 then tooclose=1;*/
/*				else tooclose=0;*/
/*		end;*/
/**/
/*	run;*/
/**/
/*	proc freq data=outc_grp1;*/
/*		table tooclose / missing;*/
/*	run;*/
/**/
/**/
/*	data outc_grp2;*/
...
/*		length RESULT_1 $20.;*/
/**/
/*		if RESULT_0 ne 'find index' then RESULT_1 = RESULT_0;*/
/*			else if OutcomeGroupsFound = 0 then RESULT_1 = RESULT_0;*/
/*			else if Dt_OutcomeGroup_End < Dt_indexprenatal then RESULT_1 = "BEFORE PREN_OC_&NUM.";*/
/*			else if index_finding_1 in ('tbd','LTFU') then RESULT_1 = 'find index';*/
/*			else if Dt_PregnancyOutcome_Start <= Dt_OutcomeGroup_End <= Dt_PregnancyOutcome_End then RESULT_1 = "PREN_OC_&NUM.";*/
/*			else if Dt_PregnancyOutcome_End < Dt_OutcomeGroup_Start and tooclose = 1 then RESULT_1 = "PREN_OC_&NUM. EXT";*/
/*			else RESULT_1 = 'find index';*/
/**/
/*	run;*/
/*	proc freq data=outc_grp2;*/
/*		table RESULT_1 / missing;*/
/*	run;*/
/*	proc freq data=outcomeassigned_pren_1;*/
/*		table RESULT_1 / missing;*/
/*	run;*/
/**/
/*	proc sort data=outc_grp2; by patient_deid outcomegrp; run;*/
/*	proc sort data=outcomeassigned_pren_1; by patient_deid outcomegrp; run;*/
/*	proc compare base=outcomeassigned_pren_1 compare=outc_grp2;*/
/*		var RESULT_1;*/
/*		with RESULT_1;*/
/*	run;*/
/*	Discussed with MEW and decided that we are okay with the find index being carried forward 
	once it is identified for a pregnancy outcome group after the indexing prenatal encounter
	Kept associated code.*/
...
%MAcro Rollup_IdxOutc;
        ...
        data ogp_result_oc;
        set ogp_updt_%eval(&NUM. - 1) (rename=(result_%eval(&NUM. - 1)=Result_OC_Fin) where=(result_oc_fin not contains "PREN_OC")) /*Final unmatched outcomegroups*/
			...
			/*Indicator that the pregnancy outcome group occurred prior to an identified pregnancy*/
			Before_Outc = (index(result_oc_fin, "BEFORE")>0);
          	...
          	keep patient_deid outcomegrp dt_outcome: Outcome_assigned&ALG. result_oc_fin prenatal_outcome postextendedtime:
               	idxpren dt_indexprenatal dt_preg: PREG_: PREG_Codetype PREG_MIFE PREG_MISO dt_PREG_Outcome days_frompregend Before_Outc;
				...
				Before_Outc = "OutcomeGrp occurred prior to an identified pregnancy";
        run; 
		*Manually reviewed potentially weird cases. Identified some unexpected values for dates. Fixed OGP_UPDT 
		macro. Now, returning rows as expected.;

		

/*		proc freq data=ogp_result_oc; table PostExtendedTime_Outc/missing; run;*/

		%*Roll up the prenatal encounter results from all of the rounds. Keep the final values;
        data pdo_result_oc ;
/*		set pdo_updt_%eval(&NUM.-1) (rename=(status_%eval(&NUM.-1)=Status_OC_Fin index_finding_%eval(&NUM.-1) = Index_Finding) where=(status_oc_fin ne 'avail'));*/
        set %do i=1 %to %eval(&NUM. - 1);
            ...
            ;
             ...
             ;
        run;
		/*Check potentially weird cases manually*/
/*		proc sql;*/
/*			create table test as */
/*			select *, count(distinct IDXPREN) as preg_count*/
/*			from ogp_result_oc*/
/*			group by patient_deid*/
/*			having calculated preg_count >1*/
/*			order by patient_deid, outcomegrp*/
/*			;*/
/*			quit;*/
		/*Looks good.*/
		...
		run;
        *want no unequal values -- CDL: 11.26.2024 after modifications to how pregnancy-level variables
					were being pulled in for the OGP_UPDT macro, this is 0. Prior to that, was getting
					different numbers of rows, at least after modifications to other lines in the
		...
		set pregs;
            ...
            if last.status then output; /*CDL: ADDED then output 11.26.2024 just to be explicit*/
...
/*		proc freq data= outcomeassigned_pren_%eval(&num. -1);*/
/*			table result_%eval(&num. -1) / missing;*/
/*		run;*/
/*		*Rows matched expected count;*/
        data opregs;
        ...
        run;
/*		Check*/
/*        proc freq data=pregs_outc;*/
/*			table result / missing;*/
/*		run;*/
/*		*Conducted manual review of rows and looked as expected. Correct number of rows in output dataset;*/
...
/*  **outcome set to missing/unknown and Last prenatal date as dt_LTFU;*/
    ...
    set  prenatal_nooutc;  
        ...
        if first.patient_deid then do;
        	...
        	if dt_indexprenatal le enc_date le dt_indexprenatal+&POSPW. /*140*/ then pno+0; /*CDL: CHANGED 140 to POSPW to match above. 11.26.2024*/
            	else if dt_indexprenatal lt enc_date le dt_indexprenatal+&POSPW. /*140*/ +28 then status=cat("PREN_NO_", pno," POST");
                else do;
...
/*    data Pregnancy_prenatalonly ;*/
	set prenatal_noOutc2;
		...
		if last.pno then output; /*CDL: ADDED then output for explicitness 11.26.2024*/
        format dt: mmddyy10.;
	...
	/*Check*/
/*	proc sql;*/
/*		select count(distinct idxpren) as expected_rows from prenatal_nooutc2;*/
/*		quit;*/
    ...
    **all prenatal dates file updated;
    ...
    set pdo_result_oc(in=a) prenatal_nooutc2 (in=b);
        ...
        if (a and status_oc_fin NOT in ("NO_OUTC", "avail" ,"LTFU")) Or b then output; /*CDL: ADDED then output for explicitness 11.26.2024*/
        PregNoOutcome = b;
        if a then Status_Final = cat("Pregnancy outcome on index prenatal #",scan(status_oc_fin,3,'_ '));
    run;
/*	*Check row count;*/
/*	proc sql;*/
/*		select count(*) as n_preen from prenatal_nooutc;*/
/*		select count(*) as n_outc from pdo_result_oc where status_oc_fin NOT in ("NO_OUTC", "avail" ,"LTFU");*/
/*		quit;*/
/*		*Expected number of rows;*/
/*	proc freq data=PrenatalDates_all_status_&OCW._&ALG.;*/
/*		table status_final;*/
/*	run;*/
...
%Macro WrapUpPreg(pods=SIMPLE) ;
    ...
    *prefer one dataset of all of the pregnancies;
    ...
    set pregnancy_indexoutc /*pregnancy_indexoutc_&ocw._&alg. */ (in=c) /*CDL: Commented out old datasets 11-13-2024 and added (in=c)*/
    	Pregnancy_PrenatalOnly_&PODS. /*pregnancy_prenatalonly_&PODS._&ocw._&alg.*/ (in=a)
        pregnancy_OutcOnly_&PODS. /*pregnancy_outconly_&PODS._&ocw._&alg.*/ (in=b);

           prenonly=a; outconly=b; prenoutc=c;
        ...
        *if preg_outcome='AEM' then preg_outcome='UAB';
        if preg_outcome='AEM' then preg_outcome_clean ='UAB'; *fix 6.18;
           ...
           label prenonly = "Pregnancy - prenatal encounters only"
				 ...
				 prenoutc = "Pregnancy - both prenatal encounters and outcome groups"
