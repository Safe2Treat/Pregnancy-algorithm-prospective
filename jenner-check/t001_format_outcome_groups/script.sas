...
/*--------------------------------------------------------------------
  Demonstration: exercise the outcome-group formats defined above on a
  small set of indicator codes, mapping each via PUT() in a DATA step.
  (Mirrors how the algorithm steps translate the 3-digit DX/PR/RX
  indicator strings into outcome-type and code-type labels.)
--------------------------------------------------------------------*/
data outcome_demo;
  length outcome $26 code $3
         abbr $3 class_ $10 codetype $4 anyflag $3;
  /* outcome long-name, evidence code (dx/pr/rx indicator) */
  outcome='Live Birth (Singleton)';  code='100'; output;
  outcome='Mixed Delivery';          code='110'; output;
  outcome='Stillbirth';              code='010'; output;
  outcome='Spontaneous Abortion';    code='001'; output;
  outcome='Unspecified Abortion';    code='011'; output;
  outcome='Ectopic/Molar';           code='000'; output;
run;

data outcome_mapped;
  set outcome_demo;
  abbr     = put(outcome, $outabbr.);   /* long name -> 3-letter abbr   */
  class_   = put(abbr,    $outclass.);  /* abbr      -> Delivery/Abortion*/
  codetype = put(code,    $outcd.);     /* code      -> DX/PR/RX/None    */
  anyflag  = put(code,    $outcyn.);    /* code      -> Yes/No           */
run;

proc print data=outcome_mapped noobs;
  var outcome abbr class_ code codetype anyflag;
run;

proc freq data=outcome_mapped;
  tables class_ codetype / nocum;
run;
