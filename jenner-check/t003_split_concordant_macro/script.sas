...
/*--------------------------------------------------------------------
  Exercise the SplitConcordant macro (which internally calls %conctbl)
  on the mock outcome groups, splitting them into concordant (resolved)
  and discordant (algorithm-bound) sets.
--------------------------------------------------------------------*/
%SplitConcordant(OutcomeGroups_30);

/* Concordant groups carry their final outcome and code type. The
   LBS+UDL group resolves through the %conctbl(LBS,UDL) pairing, whose
   cat(max(...)) over the two indicator strings yields code type 110. */
proc print data=Conc_OutcomeGroups_30 noobs;
  var patient_deid outcomegrp concordant
      outcome_concordant outcome_concordant_codetype;
run;

/* Discordant groups stay flagged not-concordant for the algorithms.
   The SAB+UAB group is correctly held back: %conctbl excludes a UAB
   identified only by a medication order ('001'/'002'/'003'). */
proc print data=Disc_OutcomeGroups_30 noobs;
  var patient_deid outcomegrp concordant outcome_concordant;
run;

proc freq data=Conc_OutcomeGroups_30;
  tables outcome_concordant / nocum;
run;
