...
/*--------------------------------------------------------------------
  Show the outcome assigned by Algorithm 1 for each group. The class is
  re-derived here with PUT($outclass.) on the assigned 3-letter code.
--------------------------------------------------------------------*/
data alg1_check;
  set OutcomeGroups_30_Alg1;
  length class_check $10;
  class_check = put(outcome_assigned1, $outclass.);
run;

proc print data=alg1_check noobs;
  var patient_deid concordant discordant1
      outcome_assigned1 outcome_class_assigned1 class_check;
run;
